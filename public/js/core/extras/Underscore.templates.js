...
(function ()
{
	...
	SPA.handleMacroError = function (error, macro_name)
	{
	...
	SPA.compileMacros = function compileMacros(macros)
	{
		...
		_.each(macros, function (macro)
		{
			...
			catch (e)
			{
				...
				SPA.handleMacroError(e, macro.substring(macro.indexOf('(') + 2, macro.indexOf(',') - 1));
			}
		});
	};
	...
	SPA.resetTemplates = resetTemplates;
	...
	,	minifyMarkup = function (text)
		{
			if (isTestingEnv() && text)
			{
				...
				if (SPA.blurInitialHTML)
				{
					// if SPA.blurInitialHTML is turned on then the @main is not wrapped with no script that's why we need to wrap image by image:
